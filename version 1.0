...
int CusFile::checkEmail(string input)
{
	for (int i = 0, n = input.length(); i < n; i++)
	{
		...
		{
			cout << "Can include only letters (a‐z, A‐Z), digits (0‐9), dot (.) and certain special  symbols !#$%&'*+-/=?^_`{|}~_ \n";
			return 9;
		}
	}
...
void CusFile::ChangeBalance()
{
	...
	for (i = 0; i < count; i++)
	{
		if (Customer[i].ID == Id)
		{
			...
			cout << "\nName of customer who has balance change: " << Customer[i].Name << endl;
...
void CusFile::ChangeEmail()
{
	...
	for (i = 0; i < count; i++)
	{
		if (Customer[i].ID == Id)
		{
			...
			cout << "\nName of customer who has email change: " << Customer[i].Name << endl;
